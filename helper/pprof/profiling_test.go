...
func cleanWorkDir() {
	if removeTempWorkDir {
		...
		if err != nil {
			fmt.Printf("Error: Couldn't remove cpu profile '%s': %s\n",
				getAbsFilePath(cpuProfilePath), err)
		}
		err = remove(blockProfilePath, 5, 2)
		if err != nil {
			fmt.Printf("Error: Couldn't remove block profile '%s': %s\n",
				getAbsFilePath(blockProfilePath), err)
		}
		err = remove(memProfilePath, 5, 2)
		if err != nil {
			fmt.Printf("Error: Couldn't remove mem profile '%s': %s\n",
	...
	if newTempWorkDir && removeTempWorkDir {
		...
		if err != nil {
			fmt.Printf("Error: Couldn't remove temp work dir '%s': %s\n",
